...
static NSString *kLinkLocalAddressKey = @"www.checkmatemobile.com/Users.asmx";
static NSString *kDefaultRouteKey = @"0.0.0.0";

static NetConnection *_sharedReachability;
...
+ (NetConnection *)sharedReachability
{
	if (!_sharedReachability) {
		_sharedReachability = [[NetConnection alloc] init];
...
- (void)scheduleOnRunLoop:(NSRunLoop *)inRunLoop
{
	...
	if ([[NetConnection sharedReachability] networkStatusNotificationsEnabled] == NO) {
		return;
	}
